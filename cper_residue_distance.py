# -*- coding: utf-8 -*-
"""Cper_residue_distance

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1hMe9kK4lvFDQbVZlZ9mHn4dU0GaKj_gY
"""
...
def load_RT_matrices(path: PosixPath | str) -> dict:
    """
    Load a JSON file containing a set of RT matrices.
...
def affine_to_transformation(affine_matx: "list[float]") -> gemmi.Transform:
    ...
    m = [
        [affine_matx[0], affine_matx[1], affine_matx[2]],
        [affine_matx[4], affine_matx[5], affine_matx[6]],
        [affine_matx[8], affine_matx[9], affine_matx[10]],
    ]
    rotation_matrix = gemmi.Mat33(m)

    # Parse affine matrix into 3x1 translation vector
    translation_vector = gemmi.Vec3(
       affine_matx[12], affine_matx[13], affine_matx[14]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mmcif1", type=str, help="Path to mmCIF file 1")
    parser.add_argument("--mmcif2", type=str, help="Path to mmCIF file 2")
    parser.add_argument("--mmcif3", type=str, help="Path to mmCIF file 3")
    parser.add_argument("--mmcif4", type=str, help="Path to mmCIF file 4")
    parser.add_argument("--rt_matrix", type=str, help="Path to JSON file containing the transformation matrix")
    parser.add_argument("--pdb_id1", type=str, help="PDB ID 1")
    parser.add_argument("--pdb_id2", type=str, help="PDB ID 2")
    parser.add_argument("--pdb_id3", type=str, help="PDB ID 3")
    parser.add_argument("--pdb_id4", type=str, help="PDB ID 4")
    parser.add_argument("--chain1", type=str, help="Chain ID 1")
    parser.add_argument("--chain2", type=str, help="Chain ID 2")
    parser.add_argument("--chain3", type=str, help="Chain ID 3")
    parser.add_argument("--chain4", type=str, help="Chain ID 4")
    ...
    entry3 = f"{args.pdb_id1}_{args.chain3}"
    entry4 = f"{args.pdb_id2}_{args.chain4}"

    # Load mmCIF files
    model1 = load_mmcif(args.mmcif1, return_model=True)
    model2 = load_mmcif(args.mmcif2, return_model=True)
    model3 = load_mmcif(args.mmcif3, return_model=True)
    model4 = load_mmcif(args.mmcif4, return_model=True)

    # Load RT matrices
    rt_matrices = load_RT_matrices(args.rt_matrix)
    # En supposant que P56817 est la clé dans le dictionnaire, ajustez cela en conséquence
    rt_matrix_key = "P56817"
    rt_matrix_values = rt_matrices.get(rt_matrix_key)

    if rt_matrix_values is None or len(rt_matrix_values) != 16:
        raise ValueError(f"Matrice RT invalide pour la clé {rt_matrix_key}")

    # Convertir la matrice RT en transformation Gemmi
    rt_matrix = affine_to_transformation(rt_matrices)

    # Apply RT matrix to models
    model1.transform_pos_and_adp(rt_matrix)
    model2.transform_pos_and_adp(rt_matrix)
    model3.transform_pos_and_adp(rt_matrix)
    model4.transform_pos_and_adp(rt_matrix)

    # Appliquer la matrice RT aux modèles
    model1.transform_pos_and_adp(rt_matrix)
    model2.transform_pos_and_adp(rt_matrix)
    model3.transform_pos_and_adp(rt_matrix)
    model4.transform_pos_and_adp(rt_matrix)
    ...
    block3 = load_mmcif(args.mmcif3, return_model=False)
    block4 = load_mmcif(args.mmcif4, return_model=False)

    unp_res_ids1 = get_unp_residue_indices(block1, args.chain1)
    unp_res_ids2 = get_unp_residue_indices(block2, args.chain2)
    unp_res_ids3 = get_unp_residue_indices(block3, args.chain3)
    unp_res_ids4 = get_unp_residue_indices(block4, args.chain4)
    all_res_ids = set()
    all_unp_res_ids = {}
    for i, j in zip((unp_res_ids1, unp_res_ids2, unp_res_ids3, unp_res_ids4), (entry1, entry2, entry3, entry4)):
    ...
    atomic_coords3 = PositionCalculator(model1[args.chain1], unp_res_ids3).get_positions()
    atomic_coords4 = PositionCalculator(model2[args.chain2], unp_res_ids4).get_positions()

    all_atomic_coords[entry1] = atomic_coords1
    all_atomic_coords[entry2] = atomic_coords2
    all_atomic_coords[entry3] = atomic_coords3
    all_atomic_coords[entry4] = atomic_coords4
    ...
    # Listes des paires d'exemples
    examples = [entry1, entry2, entry3, entry4]

    # Création d'une figure avec une grille de sous-graphiques
    fig, axs = plt.subplots(len(examples), len(examples), figsize=(15, 15), tight_layout=True)

    # Boucle à travers toutes les combinaisons d'exemples
    for i, j in product(range(len(examples)), repeat=2):
       if i == j:
        # Ignorer les combinaisons où les exemples sont les mêmes
        continue

    # Calcul des distances atomiques
    atomic_distances = calculate_atom_distances(
        all_coordinates[examples[i]],
        all_coordinates[examples[j]]
    )

    # Tracé des distances atomiques sur le sous-graphique correspondant
    axs[i, j].plot(range(0, max(all_res_ids)), atomic_distances, color="black", linewidth=0.5)
    axs[i, j].set_xlabel("UniProt Residue ID")
    axs[i, j].set_ylabel("Distance (\u212B)")
    axs[i, j].set_title(f"Euclidean Distances between {examples[i]} and {examples[j]}")

    # Affichage de la figure
    plt.show()

    # Save the plot to a file after displaying
    plot_filename = "atomic_distances_plot_Control.png"
...
# Liste pour stocker les noms des fichiers Excel filtrés
...
# Liste pour stocker les noms des fichiers Excel
excel_files = []
